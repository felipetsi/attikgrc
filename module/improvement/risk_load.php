...
if(!isset($_SESSION['user_id'])||(!isset($_SESSION['INSTANCE_ID']))){
	...
	<div class="modal-header">
		<h5 class="modal-title" id="task_boxLabel">'.$LANG_RISKS.'</h5>
	...
	<div class="modal-body" >
		<div class="row">
			<div class="col-md">
				<label class="control-label"><u>'.$LANG_RISK_No.':</u></label>
				<input class="form-control input-sm" type="text" id="especificRiskNo" 
					onkeydown="javascript:openEspecificRisk(event.keyCode,'.$ID_RELATED_ITEM.',\''.$SOURCE.'\');"
					placeholder="'.$LANG_TEXT_RISK_NUM_ENTER.'" />
			</div>
		</div>
			
		<form action="" method="post" name="risk_form" id="risk_form" autocomplete="off">
			<strong><center>'.$LANG_LAST.' (5) '.$LANG_RISKS.'</center></strong>
				...
				<thead>
					...
					<tr class="odd gradeX">
						...
						<th></th>
							...
							$SQL = "SELECT r.id, r.name, r.detail FROM trisk r WHERE r.status != 'd' ";
							if($SOURCE == "task"){	
								$SQL .= "AND r.id NOT IN (SELECT id_risk FROM tarisk_task WHERE ";
								$SQL .= "id_task = $ID_RELATED_ITEM) ";
							} elseif($SOURCE == "inci"){
								$SQL .= "AND r.id NOT IN (SELECT id_risk FROM taincident_risk WHERE ";
								$SQL .= "id_incident = $ID_RELATED_ITEM) ";
							}
							$SQL .= "AND r.id_process IN(SELECT id FROM tprocess WHERE id_area IN ";
