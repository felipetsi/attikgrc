...
### General parameters
	...
	ATTIKGRC_DIR="/tmp/attikgrc";
    POSTGRES_PASSWORD_ARM="@ttikFsadDS123Bd";

	INSTANCE_DB_NAME="attikgrc"
	INSTANCE_NAME="attikgrc"
	INSTANCE_DETAIL="@ttik GRC in Container"
	INSTANCE_LOGIN="attik"
	INSTANCE_EMAIL="attik@attik.com.br"
	INSTANCE_LIMIT_USER=-1
	INSTANCE_LANG="en"

psql -c "CREATE USER arm_user WITH PASSWORD '$POSTGRES_PASSWORD_ARM';"
psql -c "CREATE DATABASE $INSTANCE_DB_NAME WITH OWNER = arm_user;"

echo "#### End the instance data:"

cat attik-grc.sql | psql -d $INSTANCE_DB_NAME

SQL="INSERT INTO tinstance(name, detail, status, limit_user, language_default, acceptance_risk_level, limit_error_login, max_password_lifetime, min_password_lifetime, time_change_temp_password, close_system, enable_delete_cascade) VALUES ('$INSTANCE_NAME', '$INSTANCE_DETAIL', 'a', $INSTANCE_LIMIT_USER, '$INSTANCE_LANG', 2, 5, 90, 0, 1,'n','n');"
psql -d $INSTANCE_DB_NAME -c "$SQL" >> $LOGFILE;

SQL="SELECT id FROM tinstance WHERE name LIKE '$INSTANCE_NAME'";
psql -d $INSTANCE_DB_NAME -t -c "$SQL" |
while read INSTANCE 
do
	...
	if [ ! -z $ID_INST ]; 
		...
		FROM STDIN \
		DELIMITER ';' \
		CSV HEADER;" < 27001/14_nbr_sections.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER ';' \
		CSV HEADER; " < 27001/35_nbr_category.csv >> $LOGFILE;
		...
		FROM STDIN  \
		DELIMITER ';' \
		CSV HEADER;" < 27001/114_nbr_control.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER ';' \
		CSV HEADER; " < 20000/6_nbr_sections.csv >> $LOGFILE;
		...
		FROM STDIN  \
		DELIMITER ';' \
		CSV HEADER;" < 20000/22_nbr_category.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER ';' \
		CSV HEADER; " < 20000/37_nbr_control.csv >> $LOGFILE;
		...
		FROM STDIN  \
		DELIMITER ';' \
		CSV HEADER;" < 9001/7_9001_sections.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER ';' \
		CSV HEADER; " < 9001/28_9001_category.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER ';' \
		CSV HEADER;" < 9001/61_9001_control.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER '@' \
		CSV HEADER; " < BACEN/4_sections.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER '@' \
		CSV HEADER; " < BACEN/5_category.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER '@' \
		CSV HEADER; " < BACEN/10_control.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER '@' \
		CSV HEADER; " < 13709/sections.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER '@' \
		CSV HEADER;" < 13709/category.csv >> $LOGFILE;
		...
		FROM STDIN \
		DELIMITER '@' \
		CSV HEADER; " < 13709/control.csv >> $LOGFILE;
